...
intended failures become most consequential under accumulated or selective
context, where competing information dilutes attention and the model can
rationalize its own output; clean isolated checks do not establish effectiveness
under that stress. The activated agent receives a standalone decision map that
ends on observable evidence rather than activity.
