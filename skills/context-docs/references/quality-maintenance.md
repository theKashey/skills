...
2. Read the [locality ladder](locality-ladder.md) only when documentation
   ...
   surface-role, scan-path, or multi-reader alignment question is live, or
   when the Entry relevance or Published register gate, or an explanatory
   figure, is in scope. When the
...
| Entry relevance | The document opens with the reader-relevant entry need defined in [Reader entry and scan path](content-architecture.md#reader-entry-and-scan-path), including any distinction that section asks the opening or its owning section to state, while keeping the governed subject inferable. Every compact chooser or catalog entry names the subject, what it provides, and why the reader would choose it before mechanism. |
...
| Published register | On a published surface, the prose meets every rule of the [published prose register](content-architecture.md#published-prose-register). |
...
| Navigation health | Links resolve; docs build where supported; essential pages are not orphaned; facts needed to act are not available only through a visual; each explanatory figure carries one claim under [Meaningful visuals](content-architecture.md#meaningful-visuals). |
