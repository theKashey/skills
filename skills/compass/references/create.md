# Create and maintain a Compass chart
...
→ Multi-root cases and worked examples: [`blocks-and-levels.md`](blocks-and-levels.md#roots)
...
→ Registry template: [`blocks-and-levels.md`](blocks-and-levels.md#compass-registry-template)
...
→ Glossary format: [`blocks-and-levels.md`](blocks-and-levels.md#glossarymd-format)
...
→ Full procedures, product/domain evidence sources, scratchpad format: [`exploration.md`](exploration.md)
...
→ Procedure: [`exploration.md`](exploration.md#level-calibration)

## Viewports (L4)

Four viewport types — runtime, domain, boundary, lifecycle — with creation triggers and procedures in [`blocks-and-levels.md`](blocks-and-levels.md#l4--viewports).
...
→ Per-level doc formats, the consumer-relationship block, and promotion criteria: [`blocks-and-levels.md`](blocks-and-levels.md)

### Add a component

1. Open `{root}/CONTAINERS.md` and confirm which block owns the component.
...
This is an L2 semantic change and requires a human decision.

1. Check whether the candidate can remain a component in an existing block. If
   so, add a component instead.
2. Run the L2 admission criteria in
   [`blocks-and-levels.md`](blocks-and-levels.md#level-promotion-criteria) and
   the L2 gate in [`verification.md`](verification.md#l2-isolated-blocks-verification).
   Do not substitute a shorter remembered list.
3. Confirm that the block has a clear team owner.
...
- **Coordinate density points; it does not sentence.** Where coordinates refuse to bubble up, physical and logical grouping disagree — a real lead, usually toward decluttering (a stray file moved to the subtree that shares its address, a two-job file split, a helper recognized as L5). Name the Compass level and parent boundary being observed before judging the lead: the same relationship can be internal at one level and cross the highest-distance boundary at the next. Record reality first, then any healthier direction supported by independent cohesion or coupling evidence. That direction is a gradient, never an obligation; a phenomenon may legitimately span packages and services, one module may serve several phenomena, and `repository topology != semantic topology` is the normal condition — never a demand to reshape the implementation until it resembles the chart.
- **Roots are human-ratified.** Agents propose roots; agents never invent one.

→ Full rules, examples, and validation: [`coordinate-system.md`](coordinate-system.md)
...
→ Detection, response priority, and the full classification table: [`growth-and-drift.md`](growth-and-drift.md)
...
→ Both directions, the investigation flow, and worked cases: [`ownership-boundary.md`](ownership-boundary.md)
...
→ Full phase details, disagreement procedures, and rules: [`growth-and-drift.md`](growth-and-drift.md)
...
The same product can be L1 in one root and L3 in another. The test is always relative to the chosen scope and control boundary — never a fixed lookup table. Worked contrast examples: [`blocks-and-levels.md`](blocks-and-levels.md#level-promotion-criteria).

The runnable form of these rules is the L1 checklist in [`verification.md`](verification.md) — the canonical owner of all completion checklists.
...
✅ Always: read the declared chart root before chart work, and compare an installed usage hook with [the canonical template](agent-hook.md) — a missing instruction is Phase E work (`growth-and-drift.md` §Phase E); declare scope before L0; apply the rewrite test to every semantic candidate; propagate top → down; document relationships in consumers; keep within size budgets; classify disagreement before repairing it; state confidence explicitly.
